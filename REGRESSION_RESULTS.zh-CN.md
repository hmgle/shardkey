...
背景：在完成 `f49aa6b`（worker 回退与清理收紧）后，又执行了一轮最终回归复核。

覆盖点：

- 再次执行 Worker 真实生成 / 求解冒烟验证
- 再次验证静态服务下 `index.html` 与 `js/worker.js` 均可访问
- 再次确认页面与文档中的产品文案已统一为“离线问答解锁器”语义

结果：通过

关键结果：

- Worker 冒烟：正确答案可恢复，错误答案 / 不足门限均按预期失败
- 静态服务：`/` 返回 `200 OK`
- 静态服务：`/js/worker.js` 返回 `200 OK`
- 文案落点：`index.html`、`js/i18n.js`、`README.md`、`README.zh-CN.md`
